...
// `proxy`, not `middleware`). This refreshes the Supabase session cookie on
// every request and redirects unauthenticated visitors away from the
// dashboard. It is a UX convenience only — per Next's own guidance, a proxy
// matcher change can silently stop covering a route, so every Server Action
// re-checks auth itself via lib/authz.ts rather than relying on this file.
